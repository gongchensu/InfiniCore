...
Runtime::Runtime(Device device) : device_(device), graph_manager_(std::make_unique<graph::GraphManager>()) {
    activate();
    INFINICORE_CHECK_ERROR(bridge::infini::rt::translate(infini::rt::runtime::StreamCreate(&stream_)));
...
Runtime::~Runtime() {
    ...
    (void)infini::rt::runtime::StreamDestroy(stream_);
}
...
void Runtime::syncStream() {
    INFINICORE_CHECK_ERROR(bridge::infini::rt::translate(infini::rt::runtime::StreamSynchronize(stream_)));
}
...
void Runtime::memcpyH2D(void *dst, const void *src, size_t size, bool async) {
    if (async && device_.getType() != Device::Type::CPU) {
        INFINICORE_CHECK_ERROR(bridge::infini::rt::translate(infini::rt::runtime::MemcpyAsync(dst, src, size, infini::rt::runtime::kMemcpyHostToDevice, stream_)));
    } else {
...
void Runtime::memcpyD2D(void *dst, const void *src, size_t size, bool async) {
    if (async && device_.getType() != Device::Type::CPU) {
        INFINICORE_CHECK_ERROR(bridge::infini::rt::translate(infini::rt::runtime::MemcpyAsync(dst, src, size, infini::rt::runtime::kMemcpyDeviceToDevice, stream_)));
    } else {
...
void Runtime::setDeviceMemoryAsync(void *ptr, int value, size_t count, infinirtStream_t stream) {
    if (device_.getType() != Device::Type::CPU) {
        INFINICORE_CHECK_ERROR(bridge::infini::rt::translate(infini::rt::runtime::MemsetAsync(ptr, value, count, stream)));
    } else {
        INFINICORE_CHECK_ERROR(bridge::infini::rt::translate(infini::rt::runtime::Memset(ptr, value, count)));
    }
}
...
void Runtime::recordEvent(infinirtEvent_t event, infinirtStream_t stream) {
    ...
    INFINICORE_CHECK_ERROR(bridge::infini::rt::translate(infini::rt::runtime::EventRecord(event, stream)));
}
...
void Runtime::streamWaitEvent(infinirtStream_t stream, infinirtEvent_t event) {
    ...
    INFINICORE_CHECK_ERROR(bridge::infini::rt::translate(infini::rt::runtime::StreamWaitEvent(stream, event, 0)));
}
